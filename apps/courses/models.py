...
class Course(BaseModel):
    ...
    notice = models.CharField(default="", max_length=20, verbose_name="课程公告")
...
class CourseTag(BaseModel):
    ...
    weight = models.IntegerField(verbose_name="权重", default=1, choices=((1, "低"), (2, "中"), (3, "高")))
...
class Lesson(BaseModel):
    course = models.ForeignKey(Course, on_delete=models.CASCADE, verbose_name="课程")  # 当外键数据被删除后，本类型数据应如何操作。(随之删除)
...
class Video(BaseModel):
    lesson = models.ForeignKey(Lesson, on_delete=models.CASCADE, verbose_name="章节")  # 当外键数据被删除后，本类型数据应如何操作。(随之删除)
...
class CourseResource(BaseModel):
    course = models.ForeignKey(Course, on_delete=models.CASCADE)  # 当外键数据被删除后，本类型数据应如何操作。(随之删除)
