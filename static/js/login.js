...
$(function() {
    ...
    function send_sms_code(sendBtn,tipsId){
        ...
        $.ajax({
            ...
            beforeSend:function(XMLHttpRequest){
                $sendBtn.val("发送中...");  //按钮显示的数字
                $sendBtn.attr("disabled","disabled");  //设置按钮的状态为不可点击(一段时间后才能继续发送短信)
            },
            success: function(data){  //ajax发送成功有返回值时，接收试图Send_Sms_View的返回值
                $sendBtn.removeAttr("disabled");
                $sendBtn.val("发送验证码");
                if(data.mobile){  //判断返回值
    ...
    //重发送短信验证码计时
    show_send_sms = function(time){  //原来是function show_send_sms(time){}无法倒计时
